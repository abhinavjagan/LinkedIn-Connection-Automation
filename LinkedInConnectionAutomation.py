...
# Function to search and send requests
def search_and_send_request(keywords, till_page, writer, driver, ignore_list=[]):
...
# Streamlit app
def main():
    st.title("LinkedIn Connection Requests")

    # Input fields
    keywords = st.text_input("Enter Keywords", "")
    email = st.text_input("Enter Email", "")
    password = st.text_input("Enter Password", "", type="password")
    number=(st.number_input("Number of pages to search for", step=1, value=0, format="%d"))

    if st.button("Submit"):
        # Perform LinkedIn search and connection requests
        try:
            # Login
            ...
            time.sleep(10)
            # CSV file loging
            file_name = 'connection_summary.csv'
            ...
            if ignore_list:
                ...
                ignore_list = []
            # Search
            search_and_send_request(keywords=keywords, till_page=number, writer=writer,driver =driver,
